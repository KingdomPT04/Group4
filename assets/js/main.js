        ...
        // When lower Quantity is clicked
            ...
            addItem.forEach(item => {
                if (item.FoodID === id && item.Quantity >= 1) {
                    item.Quantity -= 1;
                    if (item.Quantity === 0) {
                        if (window.location != 'https://kingdompt04.github.io/Group4/user-orders.html') {
...
function userOrderManagement(trimedEmailID, userCart, userEmailID) {
    ...
    // Check AM or PM
    if (hours >= 12) { orderTime = orderTime + ' PM' } else { orderTime = orderTime + ' AM' }
...
function ClientDataFlow(addToCartBtn) {
    ...
    firebase.auth().onAuthStateChanged((user) => {
        if (user) {
            ...
            checkOutBtn.addEventListener('click', () => {
                ...
                if (addItem.length != 0) {
                    ...
                    window.setTimeout(function() {
                        window.location.replace('https://kingdompt04.github.io/Group4/user-orders.html')
                    }, 2600)
                } else {
                    Swal.fire({
                        icon: 'error',
            ...
            // Shows Orders
            if (window.location.href === 'https://kingdompt04.github.io/Group4/user-orders.html') {
